...
export function generarPdfCompras({ orders, user, t }) {
  ...
  (orders || []).forEach((order) => {
    ...
    if (order.envio?.metodo) {
      const metodoEnvio = order.envio.metodo === "pickup"
        ? (t?.misCompras?.metodoRecojo || "Retiro en tienda")
        : (t?.misCompras?.metodoEstandar || "Envío a domicilio");
      doc.text(`Envío: ${metodoEnvio}`, marginX + 8, y);
      y += 14;

      if (order.envio.metodo === "standard" && order.envio.direccion) {
        const dir = order.envio.direccion;
        const partes = [
          dir.direccion,
          dir.distrito,
          dir.zona === "provincia" ? dir.ciudad : null,
        ].filter(Boolean);
        const dirLabel = partes.join(", ");
        doc.text(`Dirección: ${dirLabel}`, marginX + 8, y);
        y += 14;
      } else if (order.envio.metodo === "pickup") {
        const dirLabel = t?.misCompras?.direccionTienda || "Tienda TechNova - Av. Principal 123, Lima";
        doc.text(`Dirección: ${dirLabel}`, marginX + 8, y);
        y += 14;
      }
    }
...
/**
 * Genera y descarga un PDF con el resumen de una sola compra (Boleta).
 * @param {object} params
 * @param {object} params.order - Orden de compra.
 * @param {object} params.user - Usuario autenticado ({ name, email }).
 * @param {object} params.t - Diccionario de traducciones activo.
 */
export function generarBoletaPdf({ order, user, t }) {
  const doc = new jsPDF({ unit: "pt", format: "a4" });
  const pageWidth = doc.internal.pageSize.getWidth();
  const pageHeight = doc.internal.pageSize.getHeight();
  const marginX = 40;
  ...
  doc.setFontSize(11);
  doc.setFont("helvetica", "normal");
  doc.text(t?.checkoutExito?.boletaTitulo || "Boleta de compra", marginX, 50);
